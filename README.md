...
- Root `Web.config`, `Global.asax` / `Global.asax.cs`: minimal, template-standard versions. `Global.asax`'s `Application_Start` is intentionally empty because nothing in the C# code needs application-level startup logic (each page gates itself via `Session` checks). The connection string name (`ConStr`) matches what every `ConfigurationManager.ConnectionStrings[...]` call reads (one file, `Student/Select_exam.aspx.cs`, reads the key as `"constr"`; that's an existing quirk in the original code, left as-is, since `ConnectionStringSettingsCollection` lookups are case-insensitive so one `ConStr` entry satisfies both).
